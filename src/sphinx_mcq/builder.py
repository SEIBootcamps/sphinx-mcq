...
class McqFeedback:
    ...
    @classmethod
    def from_node(
        cls, node: addnodes.mcq_choice_feedback, builder: Builder
    ) -> "McqFeedback":
...
class McqChoice:
    ...
    @classmethod
    def from_node(cls, node: addnodes.mcq_choice, builder: Builder) -> "McqChoice":
        ...
        not_feedback = [
            ...
            if type(child) is not addnodes.mcq_choice_feedback
        ]
...
class McqQuestion:
    ...
    @classmethod
    def from_node(cls, node: addnodes.mcq, builder: Builder) -> "McqQuestion":
        """Extract data from an mcq node."""

        # The prompt of the question is stored in mcq_body
        body_index = node.first_child_matching_class(addnodes.mcq_body)
...
class AnswerKey:
    ...
    def from_doctree(cls, doctree: nodes.document, builder: Builder) -> "AnswerKey":
        ...
        questions = []
        for mcq in doctree.traverse(addnodes.mcq):
            ...
            choices_list_items = []
            for choice_list in mcq.findall(addnodes.mcq_choices_list):
                choices_list_items = choice_list.children
