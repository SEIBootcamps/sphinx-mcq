...
class mcq_body(UsesNameAsClass, nodes.Element):
...
def visit_mcq(self, node: mcq):
    self.body.append(self.starttag(node, "div", CLASS=" ".join(node["classes"])))
...
def setup(app: "Sphinx"):
    app.add_node(mcq, html=(visit_mcq, depart_mcq))
    app.add_node(mcq_body, html=(visit_mcq, depart_mcq))
    app.add_node(
        mcq_choices_list, html=(visit_mcq_choices_list, depart_mcq_choices_list)
    )
    app.add_node(mcq_choice, html=(visit_mcq_choice, depart_mcq_choice))
    app.add_node(mcq_choice_feedback, html=(visit_mcq_choice_feedback, None))
